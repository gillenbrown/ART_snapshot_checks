...
def nbody_projection_split_species(ds, center, plot_width, axes_unit, savename):
    ...
    density_fields = [
        item
        for item in ds.derived_field_list
        if item[0] == "deposit" and "N-BODY" in item[1] and "density" in item[1]
    ]
    ...
    proj = yt.ProjectionPlot(
        ds, "x", density_fields, width=plot_width, center=center, data_source=box
    )
    # get the underlying data which we can use in imshow
    proj_data = proj.data_source.to_frb(
        width=width_tuple, height=width_tuple, resolution=n_pix
    )
    ...
    fig.suptitle("t = {:.2f} Gyr, z = {:.2f}".format(t, z), weight="bold", fontsize=24)
