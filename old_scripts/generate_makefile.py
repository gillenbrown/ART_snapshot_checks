...
def base_to_sims(base_dir):
    ...
    return [join(out_dir, item) for item in os.listdir(out_dir)
            if item.endswith(".art")]
...
if machine == "shangrila":
    ...
    sim_dirs_nbody = []
    sim_dirs_hydro = [join(runs_home, "shangrila/hui/sfe_100"),
    ...
    sim_dirs_nbody = [join(runs_home, "nbody/new_ic_trim_12.5mpc/root_05/run/outputs/fix_vel"),
    ...
    sim_dirs_hydro = [join(runs_home, "hydro/new_ic_50mpc/root_07/run/outputs/first"),
    ...
    sim_dirs_nbody = []
    sim_dirs_hydro = [join(runs_home, "hydro_test/discrete_128/run/outputs/first"),
    ...
    sim_dirs_nbody = []
    sim_dirs_hydro = ["/Users/gillenb/google_drive/research/agb/agb_wrapper/testing/stdout_tests/art_dataset"]
else:
    raise ValueError("hostname not recognized")
...
most_directories = sim_human_halos_dirs + \
                   sim_checks_dirs + \
                   sim_plots_dirs + \
...
for base_dir in sim_dirs:
    ...
    sentinel_rule = f"{halo_finding_script} " \
