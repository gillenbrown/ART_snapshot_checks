...
fig, axs = bpl.subplots(figsize=figsize, ncols=n_cols, nrows=n_rows, 
                        squeeze=False)
...
for ax, halo in zip(axs, halos):
    ...
    msun = "$M_\odot$"
    ax.easy_add_text("$M_*$ = " + exponent_format(stellar_mass, 2) + msun + "\n" + 
                     "$M_h$ = " + exponent_format(halo_mass, 2) + msun + "\n" + 
    ...
    for name, mass in zip(["stellar", "halo"], [stellar_mass, halo_mass]):
        ...
        ax.fill_between(x=ages, y1=lo_lim, y2=hi_lim, 
                        alpha=0.4, label=name + " matched")
...
fig, ax = bpl.subplots()
sms = [halo.quantities["stellar mass"].to("msun").value
       for halo in halos]
hms = [halo.quantities["particle_mass"].to("msun").value
       for halo in halos]
...
ax.legend(handles=((line, patch), scatter), labels=(um_label, sim_label),
                   loc=2, frameon=False)
